...
//use session to keep track user + cart
app.use(
    session({
...
app.all("*", function (request, response, next) {
    ...
    // need 2 initialize object to store the cart in the session. do it when there any request so that we don't have to check it exists
...
app.get("/add_to_cart/:item", function (request, response) {
    ...
    {
        // if item \already in cart,update the quanitty, else add item
...
app.get('/checkout', function (req, res, next){
    if (req.session.cart.length === 0) {
        res.redirect(
            encodeURI("products_display.html?error=Your cart = empty")
        );
...
app.get("/invoice", function (req, res, next) {
    //if cart = empty redirect to product page w/ error message
    if (req.session.cart.length === 0) {
        res.redirect(
            encodeURI("products_display.html?error=Your cart = empty")
        );
    //if the user isn't logged redirect to login
    } else if (!req.session.user) {
        console.log(req.session.cart);
        res.redirect("log-in.html");
    //mailing taken from assignment 3 examples
        ...
        transporter.sendMail(mailOptions, function (error, info) {
            if (error) {
                invoice_str +=
                    `<br><div style="text-align: right;">There was an error & your invoice couldn't be emailed :(</div><br><div style="text-align: center;"><a href="/products_display.html">Keep shopping</a></div>`;
            } else {
                invoice_str += `<br><div style="text-align: left;">Your invoice was mailed to ${user.email}</div><br><div style="text-align: center;"><a href="/products_display.html">Continue shopping</a></div>`;
            }
    ...
    checkUser(
        ...
        function (user, auth) {
            if (auth) {
                ...
                response.redirect(
                    encodeURI("/login.html?error=Incorrect username or password")
                );
            }
        }
    );
...
let registerUser = function (data) {
    //ecrypt the password and write the user
...
let validateUser = function (data) {
    ...
    if (data.username.length < 6 || data.username.length > 13) {
        return {
            success: false,
            message: "Username should be between 6 and 13 characters",
        };
    }
